...
> **Das Muster:** Plaud demonstriert das Worst-Case: CN-Firma sendet Arzt-Patienten-Audio an US-Server und US-KI. Averbis zeigt die **typische Hybridlösung**: eigene NLP souverän, generative KI auf Azure OpenAI. myScribe und Recare Voice zeigen das **Transparenzproblem**: deutsche Firmen, die KI-Backend nicht offenlegen — bei einem Startup, das Fließtext-Arztbriefe in Sekunden generiert, ist ein US-LLM im Hintergrund die wahrscheinlichste Erklärung. Die unbequeme Erkenntnis: **Kein einziger der untersuchten Anbieter für generative Gesundheits-KI (Arztbriefe, Transkription, Zusammenfassung) arbeitet nachweislich ohne US-KI-Backend.** Averbis Health Discovery (reine NLP-Extraktion) bleibt souverän — aber sobald ein System Fließtext generiert, führt der Weg aktuell zu OpenAI, Claude oder Gemini.
